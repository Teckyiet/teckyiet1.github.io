...
const itemOrderQuantity = document.querySelectorAll('.order-quantity-display input')
...
window.addEventListener('load', ()=>{
    ...
    for (let i = 0; i < savedItemID.length; i++){
        ...
        if(savedItemID.length > 0){
            ...
            itemQuantity[counter].value = localStorage.getItem('order quantity ' + counter)
...
let cartQuantityUpdate = () =>{
    ...
    cartQuantity.style.setProperty('--cartnumber', '"' + activeItemBox.length + '"')
}
